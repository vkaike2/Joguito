...
namespace Assets.Scripts.Editor
{
    ...
    public class MinToDrawer : PropertyDrawer
    {
                                   ...
                                   GUIContent label)
        {
            ...
            {
                EditorGUI.BeginChangeCheck();
                Vector2Int vec = property.vector2IntValue;
                float min = vec.x;
                float to = vec.y;
                min = EditorGUI.IntField(r[0], (int)min);
                to = EditorGUI.IntField(r[2], (int)to);
                //to = EditorGUI.IntField(position, 45);
        ...
        public static Rect[] SplitRectIn3(Rect rect)
        {
            Rect[] positionsRect = SplitRect(rect, 3);

            float initialX = positionsRect[0].x;

            // => First Value
            positionsRect[0].width /= 2;
            // => Slide Bar
            positionsRect[1].x = initialX + positionsRect[0].width;
            positionsRect[1].width *= 2;
            // => Second Value
            positionsRect[2].width /= 2;
            positionsRect[2].x += positionsRect[2].width;


            return positionsRect;
        }
